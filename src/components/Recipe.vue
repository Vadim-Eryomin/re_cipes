...
import { Image, Label, ScrollView, StackLayout, TextField, GridLayout, ActivityIndicator, alert } from '@nativescript/core'
...
const currentUserAvatar = ref('~/assets/default_avatar.jpg')
...
function toFullUrl(path: string | null | undefined): string {
  if (!path) return '~/assets/default_avatar.jpg'
...
function mapComments(commentsData: any[]) {
...
async function loadCurrentUser() {
  try {
    ...
    currentUserAvatar.value = toFullUrl(user.avatarUrl) || '~/assets/default_avatar.jpg'
  } catch (err) {
    console.error(err)
    currentUserAvatar.value = '~/assets/default_avatar.jpg'
  }
}

async function loadComments() {
  try {
    const commentsData = await postsService.getComments(props.postId)

    commentsTree.value = buildCommentsTree(mapComments(commentsData))
  } catch (err) {
...
async function loadData() {
  ...
  try {
    const postData = await postsService.getPost(props.postId)
...
async function submitComment() {
  ...
  try {
    ...
    await postsService.addComment(props.postId, text)
                ...
                <GridLayout columns="120,*">
                  <GridLayout col="0" width="120" height="100">
                    ...
                    <Label
                      ...
                      width="28"
                      height="28"
                      class="bg-orange-500 text-white text-center rounded-full m-2"
          ...
          <StackLayout class="mx-4 mt-4">
            <StackLayout orientation="horizontal" verticalAlignment="center">
              ...
              <TextField
                ...
                hint="Напишите, что думаете..."
                class="bg-gray-200 rounded-3xl ml-3 px-4"
                height="45"
                width="100%"
                @returnPress="submitComment"
              />
            </StackLayout>

            <StackLayout v-for="comment in commentsTree" :key="comment.id" class="mt-4">
              <GridLayout columns="50,*" rows="auto,auto">
                <Image rowSpan="2" col="0" :src="comment.avatar || '~/assets/default_avatar.jpg'" width="40" height="40" class="rounded-full" verticalAlignment="top" />
              ...
              >
                <StackLayout v-for="reply in comment.replies" :key="reply.id" class="mb-3">
                  <GridLayout columns="40,*" rows="auto,auto">
                    <Image rowSpan="2" col="0" :src="reply.avatar || '~/assets/default_avatar.jpg'" width="35" height="35" class="rounded-full" verticalAlignment="top" />
